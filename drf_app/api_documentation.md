...
Разрешенные методы: ```GET```  
Доступ: ```Все пользователи```  
URL: ```/api/v1/get_regions/``` - возвращает перечень всех областей Республики Беларусь.  
URL: ```/api/v1/get_city/city_id/``` - возвращает сведения об отдельном городе, переданном в качестве параметра ```city_id```.  
URL: ```/api/v1/get_cities_in_the_region/region_id/``` - возвращает перечень всех городов, входящих в область ```region_id```.